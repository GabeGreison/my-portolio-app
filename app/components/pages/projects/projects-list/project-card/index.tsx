import Image from "next/image";

export const ProjectCard = () => {
  return (
    <div className="rounded-lg h-[436px] flex flex-col bg-gray-800 overflow-hidden border-2 border-gray-800 hover:border-cyan-500 opacity-70 hover:opacity-100 transition-all group">
      <div className="w-full h-38 overflow-hidden">
        <Image
          width={380}
          height={200}
          src="https://images.unsplash.com/photo-1486312338219-ce68d2c6f44d"
          alt="thumnail do projeto"
          unoptimized
          className="w-full h-full object-cover duration-500 group-hover:scale-110 transition-all"
        />
      </div>

      <div className="flex-1 flex flex-col p-8">
        <strong className="font-medium text-gray-50/90 group-hover:text-cyan-500 transition-all">
          Projeto
        </strong>
        <p className="mt-2 text-gray-400 line-clamp-4">
          Aqui vai ficar a descricao do projeto, blablabla e etc por enquanto
          vai só isso aqui mesmo pra fazer volume, valeu um abraco
        </p>
        <span className="text-gray-300 text-sm font-medium block mt-auto truncate">
          Next.js, Typescript, Tecnologias, Mais tech{" "}
        </span>
      </div>
    </div>
  );
};
